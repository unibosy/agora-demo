...
json_parser::json_parser(std::string global_settings, std::string robot_settings)
{
    settings_.parameters.clear();

    parse_global_settings(global_settings);
    parse_robot_settings(robot_settings);
}
...
void json_parser::parse_robot_settings(std::string robot_settings)
{
    cJSON *root = nullptr;
    ...
    root = cJSON_Parse(robot_settings.c_str());
    assert(root != nullptr);
